...
test('buffers actions except passthrough', t => {
  var actionHistory = []
  ...
  setTimeout(function() {
    // history is re-ordered as expected
    t.is(actionHistory.indexOf(actionp), 0)
    t.is(actionHistory.indexOf(breaker), 1)
    t.is(actionHistory.indexOf(action1), 2)
    t.is(actionHistory.indexOf(action2), 3)
    t.is(actionHistory.indexOf(action3), 4)
    t.pass()
  }, 1000)
})
